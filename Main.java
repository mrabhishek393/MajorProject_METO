...
class Main
{
					   ...
					   double output_size)
  {
    ...
    iotDevice[]iot;
    // int number_of_indepenenttask;
    System.out.println ("enter the number of independent task : ");
	...
	if (i == 0)
	  {
	    bn =
	      both_locally_executed (local_execution_time, uplink_time,
	...
	else if (i == 1)
	  {
	    ...
	    bn =
	      first_onfog_second_locally (local_execution_time, uplink_time,
					  downlink_time, power_of_iotd1,
					  power_of_iotd2,
					  computational_power_of_fognode,
					  computational_delay, X, S, N,
	...
	else if (i == 2)
	  {
	    ...
	    bn =
	      first_locally_second_onfog (local_execution_time, uplink_time,
					  downlink_time, power_of_iotd1,
					  power_of_iotd2,
					  computational_power_of_fognode,
					  computational_delay, X, S, N,
	...
	else
	  {
	    // else 
	    bn =
	      both_onfog (local_execution_time, uplink_time, downlink_time,
	...
	iot[i].Id = i + 1;
	iot[i].dependency = 1;
	System.out.println ("enter computational demand of  task :: " +
			    (i + 1));
	...
	if (iot[i].computational_demand < 300)	//if computational demand is less than 300 then it'll be executed locally
	...
	else
	  {
	    ...
	    iot[i].latency = (iot[i].inputSize / (uplink_data_rate * 1000) +	//upload
			      (iot[i].computational_demand * iot[i].inputSize) / 50000 +	//computational
			      (10 / (downlink_data_rate * 1000)));	//download
	    iot[i].energy =
	      power_of_fogDevice * (iot[i].inputSize /
				    (uplink_data_rate * 1000) +
				    10 / (downlink_data_rate * 1000)) +
	      computational_power_of_fognode * (iot[i].computational_demand *
						iot[i].inputSize) / 50000;
	    //50000 is used as quota 
	  }
